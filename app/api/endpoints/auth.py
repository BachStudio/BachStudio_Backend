...
def google_callback(
    ...
    if error:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Google authentication error: {error_description or error}"
        )
    ...
    try:
        ...
        session = getattr(auth_response, "session", None)
        if not session:
            raise ValueError("No session returned from auth provider")
            
        frontend_base_url = settings.GOOGLE_OAUTH_REDIRECT_URL or "http://localhost:5173/auth/callback"
        access_token = session.access_token
        refresh_token = session.refresh_token
        
        final_redirect_url = f"{frontend_base_url}?access_token={access_token}&refresh_token={refresh_token}"
